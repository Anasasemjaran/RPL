...
//     header("Location: ../index.php?page=login&error=Silakan login terlebih dahulu");
//     exit();
// }
...
// 1. Ambil Data Screening Terakhir dari Database
$query = "SELECT * FROM screening_diabetes WHERE id_user = '$id_user' ORDER BY id_screening DESC LIMIT 1";
$result = mysqli_query($conn, $query);
$data_db = mysqli_fetch_assoc($result);

// 2. Tangkap Parameter dari URL (hasil dari proses_screening.php)
$skor = isset($_GET['skor']) ? (int)$_GET['skor'] : 0;
$status = isset($_GET['status']) ? $_GET['status'] : 'Tidak Diketahui';

// Jika URL tidak punya skor, coba parsing dari database (opsional sbg fallback)
if (!isset($_GET['skor']) && $data_db) {
    $status = explode(" ", $data_db['hasil_screening'])[0]; // Ambil kata pertama (Rendah/Sedang/Tinggi)
    // Ambil angka dari string "Sedang (65%)"
    preg_match('/\((.*?)\%\)/', $data_db['hasil_screening'], $match);
    $skor = isset($match[1]) ? $match[1] : 0;
}

// 3. Tentukan Warna dan Icon Dinamis Berdasarkan Tingkat Risiko
...
<section id="view-hasilAnalisis" class="view-section flex flex-col items-center flex-1 px-4 w-full max-w-5xl mx-auto py-4 animate-fade-in">
    ...
    <div class="bg-[#9c99c7] p-6 md:p-8 rounded-[2rem] w-full shadow-lg space-y-6">
        ...
        <div class="flex flex-col md:flex-row gap-4 pt-2">
            <a href="index.php?page=cek_dini" class="flex-1 bg-white text-gray-800 font-bold py-4 rounded-xl flex items-center justify-center gap-3 hover:bg-gray-50 transition-all hover:shadow-lg active:scale-95 group border border-gray-100">
